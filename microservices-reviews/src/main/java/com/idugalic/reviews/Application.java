...
public class Application extends RepositoryRestMvcConfiguration {
	...
	static class ResourceServer extends ResourceServerConfigurerAdapter {
		...
		public void configure(HttpSecurity http) throws Exception {
			http.requestMatchers().and().authorizeRequests().antMatchers(HttpMethod.GET, "/reviews/**")
					.access("#oauth2.hasScope('read_reviews') and (hasRole('ROLE_ADMIN') or hasRole('ROLE_USER'))")
